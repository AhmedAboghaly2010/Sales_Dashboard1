...
.kpi-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; padding: 20px 22px; text-align: right; position: relative; transition: transform .2s; }
.kpi-card:hover { transform: translateY(-3px); }
.kpi-card::before { content: ''; position: absolute; top: 0; right: 0; width: 4px; height: 100%; background: var(--accent-color, var(--accent1)); border-radius: 0 14px 14px 0; }
...
<div class="hero-banner">
  <p class="hero-sub">ارفع بياناتك وشاهد التحليل الفوري خطوة بخطوة للتشخيص</p>
...
# ─────────────────────────────────────────────────────────────────────────────
# DEBUGGING MODE: Buttons for each section
# ─────────────────────────────────────────────────────────────────────────────
st.info("💡 **وضع الفحص (Debug Mode):** اضغط على الزر الخاص بكل قسم لعرض محتواه وتحديد أين تكمن مشكلة التوقف.")

# KPI ROW
if st.button("📈 1. عرض المؤشرات الرئيسية (KPIs)", key="btn_kpi"):
...
# TABS
if not mg_cols:
    st.warning("⚠️ لم يتم اختيار أعمدة تصنيف. اختر الأعمدة من الشريط الجانبي لتفعيل باقي التحليلات.")
    st.stop()
...
with tabs[0]:
    if st.button("📊 2. تشغيل رسم مقارنة التصنيفات", key="btn_tab1"):
        with st.spinner("جاري رسم المخططات..."):
            ...
            for mg in mg_cols:
                ...
                st.plotly_chart(styled_fig(fig, f"📊 {mg} — أعلى {top_n} تصنيف"), width='stretch')
...
with tabs[1]:
    if not date_col:
        ...
        if st.button("📅 3. تشغيل رسم الاتجاه الزمني", key="btn_tab2"):
            with st.spinner("جاري معالجة التواريخ والرسم..."):
                ...
                for mg in mg_cols:
                    ...
                    st.markdown('<div class="chart-card">', unsafe_allow_html=True)
                    st.plotly_chart(styled_fig(fig, f"📅 {mg} — الاتجاه {view_mode}"), width='stretch')
                    st.markdown("</div>", unsafe_allow_html=True)

# TAB 3: Distribution
with tabs[2]:
    if st.button("🥧 4. تشغيل رسم التوزيع النسبي", key="btn_tab3"):
        with st.spinner("جاري رسم المخطط الدائري..."):
            ...
            for i, mg in enumerate(mg_cols):
                ...
                with cols_row[i % 2]:
                    st.markdown('<div class="chart-card">', unsafe_allow_html=True)
                    st.plotly_chart(styled_fig(fig, f"🥧 التوزيع النسبي - {mg}"), width='stretch')
                    st.markdown("</div>", unsafe_allow_html=True)

# TAB 4: Heatmap
with tabs[3]:
    if st.button("🔥 5. تشغيل خريطة التقاطع (Heatmap)", key="btn_tab4"):
        with st.spinner("جاري بناء خريطة التقاطع..."):
...
# TAB 5: Raw Data
with tabs[4]:
    if st.button("📋 6. عرض البيانات الخام", key="btn_tab5"):
